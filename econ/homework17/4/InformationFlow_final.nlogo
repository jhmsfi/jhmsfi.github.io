turtles-own[
  infoList     ;; list of available information
  turtlesList  ;; list of trust values for other turtles
  info?        ;;
]
...
to setup-turtles
  crt number-people [
    setxy random random-xcor random-ycor  ;; ******
    let a (random quantityInformation)
    set infoList n-values quantityInformation [ 0 ]
    set infoList replace-item a infoList 1
    set turtlesList n-values number-people [ random-float 0.5 ]
  ]
end
...
;; Turtles move at random.
to move ;; turtle procedure, consider adding stay
  rt random 100
  lt random 100
  fd 1
end
...
to share
  let a [who] of other turtles-here
  let y who
  let w []

  foreach a [ x -> set w ( sublist turtlesList x (x + 1) )
    if item 0 w > random-float 0.5 [
      ask turtle x [ set infoList (map [[b c]-> max list b c] [infoList] of turtle x [infoList] of turtle y)]]]
...
1150
215
Bits of Information per person
time
infoList
0.0
100.0
...
"" ""
PENS
"pen-0" 1.0 0 -7500403 true "" "ask turtles [\n  create-temporary-plot-pen (word who)\n  set-plot-pen-color color\n  plotxy ticks (sum infoList)\n]"
