...
to reset
  ask turtles [
    set belief initial-belief
    set propensity initial-propensity
  ]
  setup-colors
  reset-ticks
  set-current-plot "Average Belief"
  plot-pen-up
  plotxy 0 mean [belief] of turtles
  plot-pen-down
...
to setup-nodes
  ...
  ask turtles [
    set initial-belief random-float 1
    set initial-propensity random-float 1
  ]
end
...
to setup-spatially-clustered-network
  ...
  ask turtles
 [
   set friends-beliefs (list n-values count link-neighbors [0])
 ]
end

to setup-colors
  let monger-cut-point item ((monger-percentile / 100) * number-of-nodes) sort first (list [count link-neighbors] of turtles)
  let quasher-cut-point item ((quasher-percentile / 100) * number-of-nodes) sort first (list [count link-neighbors] of turtles)
  
  ask turtles [set color white]
  ask n-of 5 turtles with [count link-neighbors > monger-cut-point] [set color red]
  ask n-of 5 turtles with [count link-neighbors < quasher-cut-point][set color blue]
...
to go 
  if all? turtles [abs (belief - [belief] of turtle 1) < 0.05]
  [stop]
   spread-rumor
   update-beliefs
  tick
...
to spread-rumor
  ask turtles
  [
    set friends-beliefs (list [belief] of link-neighbors)
;      ask link-neighbors 
;      [
;       set [friends-beliefs] of myself fput  belief [friends-beliefs] of myself
;      ;; set friends-beliefs [belief] of myself
;      ]
  ]
    end
...
to update-beliefs
  ask turtles 
  [
    if count link-neighbors > 0 [set belief precision ( ((1 - propensity) * belief + ((propensity) * mean first friends-beliefs))) 3]
    if belief > 0.5 and belief < 1 [set color scale-color red belief 1.5 0.5]
    if belief < 0.5 and belief > 0 [set color scale-color blue belief -0.3 0.5]
  ]
end
;
...
"" ""
PENS
"pen-0" 1.0 0 -8053223 true "" "plot mean [belief] of turtles"
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" "histogram [belief] of turtles\nset-plot-y-range 0 10\nset-plot-x-range 0 1\nset-histogram-num-bars 100"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "histogram [count link-neighbors] of turtles\nset-plot-y-range 0 20\nset-plot-x-range 8 25\nset-histogram-num-bars 100"
...
<experiments>
  <experiment name="experiment" repetitions="50" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="monger-percentile">
      <value value="90"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-node-degree">
      <value value="15"/>
    </enumeratedValueSet>
    <steppedValueSet variable="quasher-percentile" first="10" step="10" last="90"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
