...
to setup-parking
  if ( pycor mod 2 = 1 )  [ set pcolor yellow ]
end

to setup-cars
 if (number_of_cars > world-width * 3)
 [
 ...
 cct number_of_cars [
   ...
   setxy random-xcor random-ycor  ;; place them randomly on the line
   set heading 90
   if (pycor mod 2 = 1)
   [
     force-unpark
...
to go
 ...
 ask turtles with [(pycor mod 2 = 1) and (state = "parking")] [decide-to-leave]
 ask turtles with [(pycor mod 2 = 0) and (state = "moving")] [decide-to-move]
 ask turtles [ move ]
 ...
 ifelse (count turtles-at 0 1) > 0 ;; the next-door space is full
  ...
  [ ;; the next-door space is empty
    ...
    [
      ...
      if (s > x) ;; the car is guarenteed a closer spot
      [
    ...
    [
      ifelse ( (s / x) * (dist / (max-pxcor + max-pycor)) > threshold)
      [ ;; we're going to go, handled by "move"
        stop
      ]
      [
...
to decide-on-row
  ;; decide what to do if car is located at the head of an aisle and can skip an entire row 
  ifelse ((count turtles-at 0 1) > 0)
  ;;someone is blocking the move up the aisle, better go down the row
  [
    stop
  ] 
  [ ;;decision process for whether to go up aisle or down row
    let srow 0 ;;talley of empty spaces
    let xrow 0 ;;talley of cars
    let rrow 1 ;;index--count up from current position
    while [rrow <= (radius - 2)]
    [ 
      if (count turtles-at rrow 3 = 0) [set srow (srow + 1)]
      if (count turtles-at rrow 2 = 1) [set xrow (xrow + 1)]
      ifelse (srow > xrow) ;; the car is guarenteed a closer spot
      [ ;; we're going to go to the next row
        set heading 0
        fd 2
        set heading 90
        stop
    ...
    ifelse (xrow != 0)
    [ 
      ifelse ( (srow / xrow) * (dist / (max-pxcor + max-pycor)) > threshold)
      [ ;; we're going to go to the next row
        set heading 0
        fd 2
        set heading 90         
...
to move
 ifelse ((pycor mod 2 = 1) and (state = "leaving")) [ unpark ]
 [
   if ((pycor mod 2 = 0))
   [
...
to record-data
 ...
 set total_distance (total_distance + dist)
 ...
 let utility (walking_cost * (max-pxcor - pxcor) + driving_time_cost * time_driving + regret)
...
to-report avg_driving_time
 ifelse (total_driving_time = 0) 
 [
   report 0
 ]
 [
...
to calc-dist
  let xsqchange ((max-pxcor - pxcor) ^ 2)
  let ysqchange ((max-pycor - pycor) ^ 2)
  set dist sqrt(xsqchange + ysqchange)
end
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryTick="false">
    ...
    <go>go</go>
    <timeLimit ticks="500"/>
    <steppedValueSet variable="default_threshold" first="0.0" step="0.05" last="1"/>
    ...
    <enumeratedValueSet variable="number_of_cars">
      <value value="15"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment" repetitions="3" runMetricsEveryTick="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit ticks="5000"/>
    <metric>avg_distance</metric>
    <metric>avg_driving_time</metric>
